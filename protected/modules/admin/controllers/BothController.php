...
class BothController extends AdminController
    {
            ...
            array(// display a column with "update" and "delete" buttons
                ...
                'buttons' => array
                    ...
                    'Approval' => array
                        ...
                        'url' => 'Yii::app()->createUrl("admin/both/cancelapprove/", array("id"=>$data->id))',
                    ),
                    'Pending Approval' => array
                        ...
                        'url' => 'Yii::app()->createUrl("admin/both/approve/", array("id"=>$data->id))',
                    ),
                )
            ),
        );
...
//         $this->performAjaxValidation($model);
        ...
        if (isset($_POST['UserMaster']))
            ...
            if ($model->validate())
                ...
                $model->save();
                Yii::app()->user->setFlash('success_msg', "Employer / Designer Updated Successfully");
    ...
    public function actionApprove($id) {
        ...
        if ($model->status == 0)
            {
            $email_body = EmailNotification::model()->findByAttributes(array('email_code' => 'both_pending_to_approve'));
            }
        elseif ($model->status == 2)
            {
            $email_body = EmailNotification::model()->findByAttributes(array('email_code' => 'both_inactive_to_active'));
            }
        ...
        if ($model->save(FALSE))
            ...
            $res['status'] = '<a class="admin_processing glyphicon glyphicon-ok" title="Do Inactive" rel="tooltip" href="' . Yii::app()->request->baseUrl . '/admin/both/cancelapprove/id/' . $model->id . '" data-original-title=""></a>';
    ...
    public function actionCancelapprove($id) {
        ...
        if ($model->save(FALSE))
            ...
            $res['status'] = '<a class="admin_processing1 glyphicon glyphicon-remove" title="Do Active" rel="tooltip" href="' . Yii::app()->request->baseUrl . '/admin/both/approve/id/' . $model->id . '" data-original-title=""></a>';

            $email_body = EmailNotification::model()->findByAttributes(array('email_code' => 'both_inactive'));
            $name = $model->username;
