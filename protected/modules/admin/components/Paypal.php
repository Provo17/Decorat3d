...
class Paypal extends CApplicationComponent
{
    ...
    {
//        $objPaypalApiUsername = SiteSettings::model()->findByAttributes(array('slug'=>'paypal_apiUsername'));
//        $this->username = $objPaypalApiUsername->value;
//        $objPaypalApiPassword = SiteSettings::model()->findByAttributes(array('slug'=>'paypal_apiPassword'));
//        $this->password = $objPaypalApiPassword->value;
//        $objPaypalApiSignature = SiteSettings::model()->findByAttributes(array('slug'=>'paypal_apiSignature'));
//        $this->signature = $objPaypalApiSignature->value;
//        $objPaypalApiLive = SiteSettings::model()->findByAttributes(array('slug'=>'paypal_apiLive'));
        
        $objPaypalApiUsername = Settings::model()->findByAttributes(array('slug' => 'paypal_apiUsername')); //SiteSettings::model()->findByAttributes(array('slug'=>'paypal_apiUsername'));
        $this->username = $objPaypalApiUsername->value;
        $objPaypalApiPassword = Settings::model()->findByAttributes(array('slug' => 'paypal_apiPassword')); //SiteSettings::model()->findByAttributes(array('slug'=>'paypal_apiPassword'));
        $this->password = $objPaypalApiPassword->value;
        $objPaypalApiSignature = Settings::model()->findByAttributes(array('slug' => 'paypal_apiSignature')); //SiteSettings::model()->findByAttributes(array('slug'=>'paypal_apiSignature'));
        $this->signature = $objPaypalApiSignature->value;
        $objPaypalApiLive = Settings::model()->findByAttributes(array('slug' => 'paypal_apiLive')); //SiteSettings::model()->findByAttributes(array('slug'=>'paypal_apiLive'));
        
        
        
        if($objPaypalApiLive->value==1){
            $this->sandbox = false;
        }else{
            $this->sandbox = true;
        }
        ...
        //$this->returnUrl = Yii::app()->createAbsoluteUrl('/admin/WithdrawRequest/success/');
        //$this->cancelUrl = Yii::app()->createAbsoluteUrl('/admin/WithdrawRequest/cancel/');
    }
