...
class EDataTables extends CGridView {
    ...
    protected function restoreStateSessionInternal() {
        ...
        if (!($sort instanceof EDTSort) || !($pagination instanceof EDTPagination))
            return;
        ...
        if ($this->selectableRows) {
            ...
            foreach ($specialFields as $field) {
                ...
                echo CHtml::hiddenField(
                        $this->getId() . '-' . $field, $this->unsavedChanges[$field], array('id' => $this->getId() . '-' . $field)
                );
            }
        }
        if (isset($this->unsavedChanges['values']))
            echo CHtml::hiddenField(
                    $this->getId() . '-values', $this->unsavedChanges['values'], array('id' => $this->getId() . '-values')
            );
    }
