...
//        $this->apiUsername = 'sudipta.mal-facilitator_api1.infoway.us';
...
//        $this->apiPassword = 'KDHENN8EGC7HMVRK';
...
//        $this->apiSignature = 'Ak8DDwiRyDyJBRJFXF4taPDp4AooAelm.FD0FGeb9Oc2JbMv1IvCWJ1R';
//        $objPaypalApiLive = Settings::model()->findByAttributes(array('slug' => 'paypal_apiLive')); //SiteSettings::model()->findByAttributes(array('slug'=>'paypal_apiLive'));
        ...
        $objPaypalApiUsername = Settings::model()->findByAttributes(array('slug' => 'paypal_apiUsername')); //SiteSettings::model()->findByAttributes(array('slug'=>'paypal_apiUsername'));
        $this->apiUsername = $objPaypalApiUsername->value;
        $objPaypalApiPassword = Settings::model()->findByAttributes(array('slug' => 'paypal_apiPassword')); //SiteSettings::model()->findByAttributes(array('slug'=>'paypal_apiPassword'));
        $this->apiPassword = $objPaypalApiPassword->value;
        $objPaypalApiSignature = Settings::model()->findByAttributes(array('slug' => 'paypal_apiSignature')); //SiteSettings::model()->findByAttributes(array('slug'=>'paypal_apiSignature'));
        $this->apiSignature = $objPaypalApiSignature->value;
        $objPaypalApiLive = Settings::model()->findByAttributes(array('slug' => 'paypal_apiLive')); //SiteSettings::model()->findByAttributes(array('slug'=>'paypal_apiLive'));
    ...
    public function DoDirectPayment($paymentInfo = array()) {
         ...
         */
        $paymentType = urlencode('Sale') ? urlencode('Sale') : '';
        $firstName = urlencode($paymentInfo['CreditCard']['first_name']) ? urlencode($paymentInfo['CreditCard']['first_name']) : '';
//        $lastName =urlencode($paymentInfo['Member']['last_name']) ? urlencode($paymentInfo['Member']['last_name']) : ''; 
        $creditCardType = urlencode($paymentInfo['CreditCard']['credit_type']) ? urlencode($paymentInfo['CreditCard']['credit_type']) : '';
        $creditCardNumber = urlencode($paymentInfo['CreditCard']['card_number']) ? urlencode($paymentInfo['CreditCard']['card_number']) : '';
        $expDateMonth = urlencode($paymentInfo['CreditCard']['expiration_month']) ? urlencode($paymentInfo['CreditCard']['expiration_month']) : '';
        $padDateMonth = str_pad($expDateMonth, 2, '0', STR_PAD_LEFT);
        $expDateYear = urlencode($paymentInfo['CreditCard']['expiration_year']) ? urlencode($paymentInfo['CreditCard']['expiration_year']) : '';
        $cvv2Number = urlencode($paymentInfo['CreditCard']['cv_code']) ? urlencode($paymentInfo['CreditCard']['cv_code']) : '';
//        $address1 = urlencode($paymentInfo['Member']['billing_address']) ? urlencode($paymentInfo['Member']['billing_address']) : ''; 
//        $address2 = urlencode($paymentInfo['Member']['billing_address2']) ? urlencode($paymentInfo['Member']['billing_address2']) : ''; 
//        $country = urlencode($paymentInfo['Member']['billing_country']) ? urlencode($paymentInfo['Member']['billing_country']) : ''; 
//        $city = urlencode($paymentInfo['Member']['billing_city']) ? urlencode($paymentInfo['Member']['billing_city']) : ''; 
//        $state = urlencode($paymentInfo['Member']['billing_state']) ? urlencode($paymentInfo['Member']['billing_state']) : ''; 
//        $zip = urlencode($paymentInfo['Member']['billing_zip']) ? urlencode($paymentInfo['Member']['billing_zip']) : ''; 
...
//        "&ZIP=$zip&COUNTRYCODE=$country&CURRENCYCODE=$currencyCode";
        ...
        $nvpstr = "&PAYMENTACTION=Sale&IPADDRESS=$ip&AMT=$amount&CREDITCARDTYPE=$creditCardType&ACCT=$creditCardNumber&EXPDATE=" . $padDateMonth . $expDateYear . "&CVV2=$cvv2Number&FIRSTNAME=$firstName" ;
//                "&CURRENCYCODE=$currencyCode'&RETURNURL='.$returnURL.'&CANCELURL='.$cancelURL.";
