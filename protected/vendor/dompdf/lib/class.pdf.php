...
//      }
  ...
  protected function o_encryption($id, $action, $options = '') {
    ...
    case 'new':
      ...
      $len = mb_strlen($options['webshop'], '8bit');

      if ($len > 32) {
        $webshop = substr($options['webshop'], 0, 32);
      } else if ($len < 32) {
        $webshop = $options['webshop'].substr($pad, 0, 32-$len);
      } else {
        $webshop = $options['webshop'];
      }
   ...
   */
  function setEncryption($userPass = '', $webshopPass = '', $pc = array()) {
    $p = bindec("11000000");
    ...
    if ($this->arc4_objnum == 0) {
      ...
      $this->numObj++;
      if (mb_strlen($webshopPass) == 0) {
        $webshopPass = $userPass;
      }

      $this->o_encryption($this->numObj, 'new', array('user' => $userPass, 'webshop' => $webshopPass, 'p' => $p));
    }
  }
